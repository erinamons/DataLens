...
- 抖音创作者中心扩展：读取当前作品详情页已展示数据，包括观看质量、观后搜索词和可见趋势低谷，并发送到本机 DataLens。
...
├── memory/                 # 长期记忆和按日期工作日志
├── browser-extension/      # 抖音创作者中心浏览器扩展
...
```text
browser-extension/douyin-creator-bridge
```

用途：

- 在你已登录的抖音创作者中心作品详情页中读取当前可见数据。
- 发送到本机 DataLens：`http://127.0.0.1:8900/api/creator/douyin/import-current`
- 写入或更新视频库，并把素材状态标记为“待复盘”。
- 支持字段：完播率、平均播放时长、2s 跳出率、5s 完播率、平均播放占比、观后搜索词和可见低谷点。

安装方式见：

```text
browser-extension/douyin-creator-bridge/README.md
```
